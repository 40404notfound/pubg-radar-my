...
class GLMap: InputAdapter(), ApplicationListener, GameListener {
  ...
  lateinit var rareFont: BitmapFont
  lateinit var medFont: BitmapFont
  ...
  override fun create() {
    ...
    alarmSound = Gdx.audio.newSound(Gdx.files.internal("warning.wav"))
    mapErangel = Texture(Gdx.files.internal("map1.bmp"))
    mapMiramar = Texture(Gdx.files.internal("map2.bmp"))
    map = mapErangel
    ...
    param.color = BLACK
    param.size = 15
    rareFont = generator.generateFont(param)//todo

    param.color = BLACK
    param.size = 15
    medFont = generator.generateFont(param)//todo
